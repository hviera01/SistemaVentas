...
class _ColoresScreenState extends ConsumerState<ColoresScreen> {
  ...
  void _buscar() {
    ref
        .read(coloresBusquedaProvider.notifier)
  ...
  void _abrirFormulario([ColorModel? color]) {
    showDialog(
      ...
      context: context,
      builder: (context) => ColorFormDialog(color: color),
    );
  }
  ...
  void _abrirImportar() {
    showDialog(
      useRootNavigator: false,
      context: context,
      builder: (context) => const ImportarColoresDialog(),
    );
  }
                      ...
                      OutlinedButton.icon(
                        onPressed: () => Navigator.of(context).push(
                          MaterialPageRoute(
                            ...
                            builder: (context) => const BuscarFormulaScreen(),
                          ),
                        ),
                  ...
                  data: (colores) {
                    ...
                    return [
                      DecoratedSliver(
                        ...
                        sliver: esMovil
                            ? _tarjetasSliver(lista)
                            : _tablaSliver(lista, constraints.maxWidth),
                      ),
                    ];
                  },
                  loading: () => [
      ...
      slivers: [
        ...
        SliverList(
          delegate: SliverChildBuilderDelegate((context, index) {
            ...
            final ubicacion = [
              color.ubicacionFisica,
              if (color.pagina.isNotEmpty) 'Pág. ${color.pagina}',
            ...
            return Column(
              children: [
                ...
                InkWell(
                  onTap: () => setState(
                    () => _filaSeleccionada = seleccionada ? null : color.id,
                  ),
                  child: Container(
                    color: seleccionada
                    ...
                    child: Row(
                      children: [
                        _celda(
                          2,
                          color.codigo.isEmpty ? '-' : color.codigo,
                        ...
                        if (mostrarDescripcion)
                          _celda(
                            4,
                            color.descripcion.isEmpty ? '-' : color.descripcion,
                            gris: true,
                          ),
                        if (mostrarUbicacion)
                          _celda(
                            2,
                            ubicacion.isEmpty ? '-' : ubicacion,
                            gris: true,
                          ),
                        _celda(
                          2,
                          color.fechaRegistro != null
                              ? formatoFecha.format(color.fechaRegistro!)
                              : '-',
                          gris: true,
                        ),
  ...
  Widget _tarjetaColor(ColorModel color, DateFormat formatoFecha) {
    ...
    return InkWell(
      borderRadius: BorderRadius.circular(16),
      onTap: () =>
          setState(() => _filaSeleccionada = seleccionada ? null : color.id),
      child: Container(
